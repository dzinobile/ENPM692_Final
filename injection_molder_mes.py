...
BUILD_ORDER_FIELDS = ["build_number", "build_name", "build_quantity", "parts_completed", "build_status"]
...
def load_build_order(build_number: str) -> dict | None:
    """Return the matching row from build_orders.csv or None if not found."""
    if not os.path.exists(BUILD_ORDERS_FILE):
        return None
    with open(BUILD_ORDERS_FILE, newline="") as f:
        for row in csv.DictReader(f):
            if row["build_number"].strip() == build_number.strip():
                return row
    return None


def update_build_order(build_number: str, parts_completed: int) -> dict:
    """Overwrite parts_completed and recalculate build_status for a build order row."""
    rows = []
    updated = None
    with open(BUILD_ORDERS_FILE, newline="") as f:
        ...
        for row in reader:
            if row["build_number"].strip() == build_number.strip():
                qty = int(row["build_quantity"])
                if parts_completed == 0:
                    row["build_status"] = "Not Started"
                elif parts_completed < qty:
                    row["build_status"] = "In Progress"
                else:
                    row["build_status"] = "COMPLETE"
                row["parts_completed"] = str(parts_completed)
                updated = row
            rows.append(row)
    with open(BUILD_ORDERS_FILE, "w", newline="") as f:
        writer = csv.DictWriter(f, fieldnames=BUILD_ORDER_FIELDS)
        writer.writeheader()
...
class MESApp(tk.Tk):
    ...
    def _scan_build_info(self):
        ...
        qty       = int(order["build_quantity"])
        completed = int(order["parts_completed"])

        self.build_name.set(order["build_name"])
        self.build_quantity.set(f"{completed} / {qty}")
        self.build_status.set(order["build_status"])
        self._refresh_build_status_color(order["build_status"])
        ...
        self._status(f"Build {build_num} — {order['build_name']} loaded.")
        self._append_feed(row)
    ...
    def _increment_part(self):
        ...
        # Write back to build_orders.csv and refresh status display
        build_num = self.build_number.get().strip()
        if build_num:
            updated = update_build_order(build_num, new_count)
            if updated:
                qty = int(updated["build_quantity"])
                self.build_quantity.set(f"{new_count} / {qty}")
                self.build_status.set(updated["build_status"])
                self._refresh_build_status_color(updated["build_status"])
